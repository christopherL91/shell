...
char* read_input() {
    char* line;
    char prompt[1024] = ANSI_COLOR_MAGENTA "\\ʕ◔ϖ◔ʔ/ : "
        ANSI_COLOR_RESET ANSI_COLOR_BLUE UNDERSCORE;
    char* cwd;
    char* directory_name;

    /*Get current directory*/
    cwd = getcwd(0, 0);
    directory_name = (char*)getCurrentDir(cwd);
     ...
     *  command prompt
    */
    line = readline(strcat(prompt, ANSI_COLOR_GREEN " >> "
        ANSI_COLOR_RESET));

    /*TODO do a rewrite of these lines.*/
    if(line != NULL) {
        if(line[0] != 0) {
            add_history(line);
        }
